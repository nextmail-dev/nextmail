...
6. 多账户：账户管理、Supervisor、并发限制、独立同步策略和账户层级导航。（已完成实施计划，待确认后实施）
7. POP3：TLS/STARTTLS/非加密、UIDL、服务器副本策略和真实服务商兼容门禁。
8. 模板与签名：作用域、变量替换、可可靠替换的签名节点，以及新建/回复/转发场景模板；基础回复与转发已在第四阶段提供。
9. Google 与 Microsoft OAuth：系统浏览器、PKCE、回调、刷新、撤销和重新认证。
10. 搜索、会话与桌面集成：FTS5、会话聚合、托盘、未读数、通知和窗口行为。
11. 迁移、发布与硬化：跨机重绑定、性能和损坏恢复、安全审计、签名、公证、发布与自动更新。
