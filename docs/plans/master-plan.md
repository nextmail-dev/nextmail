...
HTML 邮件由 Rust 白名单清洗，并在无 scripts、forms、same-origin 和 top-navigation 权限的 sandbox iframe 中显示。安全的 `http`、`https`、`mailto` 链接现由主 WebView 回调再次校验后直接交给系统关联程序，外部网页不在 NextMail 内创建；传统表格邮件布局属性与 class/ID 已纳入安全保真边界。远程图片 URL 仍由严格 CSP 默认阻止，CSS 网络资源与阅读器 CID 展示仍未实现；Composer 已能缓存原始 MIME CID 图片与粘贴图片，并以 `multipart/related` 发件。
...
11. 搜索、会话与桌面集成：FTS5、会话聚合、托盘、未读数、通知和窗口行为。（实施中；第一批当前文件夹本地全文搜索已验收，第二批会话聚合尚未实施）
