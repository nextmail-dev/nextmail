...
本文是 NextMail 新会话唯一必读的长期技术文档，集中保存当前实现事实、开发细则、工程约定和项目记忆。截止 `2026-08-21-02` 的历史计划保留在 `iterations/`，之后的简短开发记录统一写入按日期倒序排列的 [`iterations.md`](./iterations.md)；重大架构或安全取舍保留在 `adr/`，都只在任务相关时按需查阅。
...
5. 不再创建开发计划；完成开发后，在 [`iterations.md`](./iterations.md) 顶部的当天日期区块记录简短的 `- [ ]` 待办项，日期按倒序排列；只有用户明确验收后才改为 `- [x]`。
6. 不得 reset、覆盖、清理或顺手提交用户已有修改；出现重叠时先说明。
7. 按风险完成验证，交付结果和必要的实机验收步骤。

## 2. 产品与当前状态

NextMail 是基于 Tauri 2、React/TypeScript 和 Rust 的本地优先桌面邮件客户端，当前版本为 `0.9.0`。
...
- SQLite 离线视图、全服务器邮件头与有界文字预览、`BODYSTRUCTURE` 优先同步、按 MIME section 获取完整正文、账户可选正文预取和逐封可见更新。
...
- 原始 EML 按需下载、查看与单封另存为，附件元数据先行与 section 按需下载、安全另存为、受控系统打开和所在文件夹定位。
- 当前账户下支持当前文件夹或全部可选文件夹范围的本地搜索；Enter 或搜索按钮显式提交。只匹配主题、收发件人和纯文本正文：3 个及以上 Unicode 字符使用 SQLite FTS5 trigram `MATCH`，1–2 字符按时间索引从目标范围的新邮件向旧邮件检查并在收满一页后停止，避免先扫描全账户 FTS 虚表；短查询没有匹配或匹配极少时仍可能检查完整目标范围。
...
- 隐藏演示模式已实现，待实机验收：设置 → 关于 → 应用信息的 NextMail 标题连续点击 11 次后确认进入（相邻点击不超过 2 秒，点击其他位置重置）。进入前需关闭写信、模板与签名编辑窗口，防止丢失未保存内容；确认后关闭其他业务窗口并重载主窗口，清除真实 Query 缓存、选择与回调。演示账户、联系人及 12 封不同类型邮件均为双语虚构商务内容，展示身份统一使用 `next-mail.app` 域名；已读、星标、搜索、移动/复制/归档/删除及文件夹收藏等只操作内存。账户管理、文件访问、真实设置与独立邮件窗口在演示期间不可用；新建邮件按钮保持正常外观，点击时静默忽略。
- 演示状态和语言偏好仅保存在 Rust 进程内，WebView 启动先读取状态再挂载业务 UI；`api.ts` 经演示适配层读取内存数据，Rust Command 入口另以允许列表阻止访问真实业务。演示 HTML 仍经现有 Rust 权威清洗器与原有 sandbox iframe。进入时停止后续收发调度、屏蔽真实邮件事件与通知；已经开始的真实网络操作允许完成收尾，不中途取消已提交 SMTP 发送。Ctrl+L（macOS 同时支持 Cmd+L）呼出语言选择框，并切换界面与演示文案；原生菜单 accelerator 覆盖 iframe 焦点，未放宽邮件脚本权限。只能完全退出进程并重新打开恢复正常，关闭到托盘或重新聚焦不会退出演示。

- `0.9.0` 已完成并发布：同步新邮件时批量取得有界纯文本预览，没有纯文本时使用稳定占位符，完整正文与附件继续按需下载；前端完成资源与调用入口拆分、默认跟随系统主题，并统一图标主题色和标题栏外观。
- `0.8.7` 已完成并通过验收：新增进程级隐藏演示模式与演示语言切换；IMAP 完整同步扩展为三条动态工作会话和一条交互会话，健康连接可短时复用；非标准 `BODYSTRUCTURE` 自动降级恢复；邮件列表使用虚拟化并移除待同步云端图标。
- `0.8.6` 已完成并通过验收：联系人页新增账户内分组管理，支持创建、改名、搜索选择成员与删除；Composer 和模板的 To/Cc/Bcc 地址补全支持整组加入并去重。
- `0.8.5` 已完成并通过验收：HTML 正文滚动条移入右侧留白并使用常显主题滑块；文件夹与邮件列表滑块在留白内居中，分栏拖拽命中区收窄并与滚动条操作区分离。
- `0.8.4` 已完成并通过验收：邮件正文中的超宽图片按阅读视口等比例缩小；邮件列表右键菜单和邮件详情右上角更多菜单支持将单封原始邮件另存为 EML。
- `0.8.3` 已完成并通过验收：Composer 富文本编辑、HTML 预览与实际发件正文复用统一样式，默认保持紧凑的 `14px / 1.2` 排版，显式格式继续覆盖默认值，旧草稿发件时自动补齐统一样式且引用原文保持独立。
- `0.8.2` 已完成并通过验收：从邮件中编辑联系人时保持当前邮件详情；正文尚未收取完成时不再提前展示可能被正文引用的图片附件，正文与 CID 判定完成后再显示最终附件。
- `0.8.1` 已完成并通过验收：支持识别并展示发送方标记的高优先级邮件；本地搜索收敛到主题、收发件人和正文，支持当前文件夹与当前账户全局范围切换，并改善大型文件夹中的短字符搜索；从邮件地址编辑联系人时直接弹出编辑器，不再切换联系人页面或等待往来邮件载入。
- `0.8.0` 已完成并通过实机验收：侧栏新增可持久化的真实文件夹收藏，以及固定的星标邮件、未读邮件聚合入口；邮件列表支持直接切换已读状态，未读聚合视图会把本次打开后已读的邮件保留到离开视图；文件夹与邮件列表支持方向键选择，并按账户恢复最后选中的文件夹；同步提示、滚动条、文件夹点击区域与对齐得到收敛；修复 HTML-only 邮件混入文档标题、重复点击当前文件夹清空阅读状态的问题，并改善新附件大小估算。
- `0.7.1` 已完成并通过 Windows 实机验收：新写邮件自动插入的默认签名与正文之间补上细分隔线；模板插入不再套用编辑器盒样式、按原样呈现；Composer 默认行距得到收紧；未下载附件在大小右侧显示云朵标记，另存为改为先选择保存位置再下载（取消不产生下载），转圈只在附件内容实际传输时出现。
...
- 规则、日历、PGP/S-MIME、EML/MBOX 导入与批量导出和 Linux 深度适配。
...
```text
src/
  app/                  IPC 门面、DTO、Query key、外观、语言、平台入口
  app/api/              按账户、邮件、联系人、写信、偏好和生命周期拆分的 IPC 实现
  ...
  locales/              zh-CN、en-US；每种语言按业务域拆分并由 index.ts 聚合
  styles/               语义主题与全局样式
src-tauri/
...
docs/iterations.md       按日期倒序排列的简短开发记录
...
- 业务组件统一通过 `src/app/api.ts` 和稳定 DTO 调用 Command，不散落裸 `invoke`；`api.ts` 只保留稳定公共门面，具体调用按业务域放在 `src/app/api/`。
...
- 中英文资源分别按通用、初始化、邮件、联系人、写信、账户、设置和错误域拆分，数量较多的错误文案继续按账户、邮件、写信、联系人和系统分类；同一语言的 `index.ts` 聚合为既有翻译 key，业务组件无需感知文件边界。测试会忽略语言特有复数后缀后校验两种语言的 key 集合。
...
- 基础设施错误统一通过 `diagnostics::command_error`（共享转换器保留调用位置）生成安全诊断：仅记录稳定错误码、错误类型、原因分类、OS/SQLite 数字错误码和 Rust 来源位置。权限不足、文件占用、磁盘满、数据库繁忙、连接/协议/凭据等原因以允许列表中的 `params.reason` 供 UI 翻译；业务校验继续使用稳定错误码。`CommandError` 的 Debug 不含 params，日志 observer 也不接收 params。
- `api.ts` 统一规范化和记录失败；所有错误展示使用 `formatCommandError`，保留安全原因并显示可供排查的稳定错误码，中英文错误码覆盖由源码扫描测试保护。后台记忆文件夹位置的连续同类失败只提示一次，成功保存后解除抑制；主动操作的错误仍在原表单或页面提示。读取仅对 `retryable` 错误重试一次且不因窗口重新聚焦而自动重取，写操作不自动重试。
...
- 应用按标识符保持单实例：再次启动时第二个进程退出，第一个进程显示、还原并聚焦主窗口；开发构建与安装版共用同一标识，两者同时运行受同一约束。
...
同步遍历全部可选文件夹，每条邮件头 `UID FETCH` 最多包含 20 UID，并直接逐项消费 IMAP 响应流；每收到一封就解析并原子落库，批次中途断线时已收到的邮件不会丢失，下次按本地已有 UID 差集续传。随后用原有批量 `BODYSTRUCTURE` 确认真实 `text/plain` section，把同一批中 section 路径相同的 UID 合并到一次预览 FETCH；常见邮件每 20 封只增加一条请求，只有 section 路径不同时才拆分请求。预览只读取已验证纯文本 section 的 MIME 头和最多 8 KiB 内容，不读取 HTML 或附件；它只生成最多 180 字符的列表预览，不写入 `message_bodies`、不把完整正文标记为可用。服务器未返回、无法解析、没有纯文本 section 或 `BODYSTRUCTURE` 兼容降级时使用固定单行占位符 `—`。新邮件事件在本批预览完成或确定使用占位符后发布，因此前端加入邮件时始终已有非空、稳定高度的预览。同步开始后先把 `LIST` 返回的全部文件夹用 `ensure_mailbox`（只插入缺失行，不覆盖既有同步元数据）一次性建齐并发布变更事件，侧栏立即显示完整目录结构，再逐个文件夹同步邮件。邮件头批次进入共享动态队列，三个同步 worker 完成当前批次后立即领取下一批，避免静态连续分片中快连接等待慢连接。`BODYSTRUCTURE` 仍与头部分开，防止服务器返回语法不规范的结构（如 transfer encoding 字段为 NIL）污染响应流并丢失已收到的邮件头；发生此类错误时立即废弃整个连接池并在同一轮用新连接恢复，当前进程内对该账户禁用后续 `BODYSTRUCTURE`，避免把坏连接带入下一文件夹并误判为不支持 UID。已落库邮件头按 UID 差集复用；降级账户打开邮件时直接走完整正文回退，启用自动下载正文时也直接抓取完整邮件。当前邮箱的到达事件进入最多 256 封的前端有界队列，并按动画帧逐封写入最多 50 封的首页缓存。超过上限时丢弃中间展示队列并从数据库重读一次活动首页，不丢已落库邮件。其他邮箱的到达事件按帧合并失效；联系人变化每 300ms 合并，邮件列表和详情只标记陈旧而不主动重取。同步进度仍按账户约 100ms 合并写入查询缓存并按 revision 拒绝迟到事件。进度订阅只位于侧栏子树，主窗口各窗格组件 memo 化且回调稳定；邮件列表使用虚拟化且只挂载可视行和 8 行 overscan，拖拽分隔条只重渲染布局壳层。

默认同步邮件头、有界文字预览和 `BODYSTRUCTURE`，在同一落库事务保存附件文件名、类型、大小和稳定 IMAP section，不下载附件内容。`BODYSTRUCTURE` 对 `BASE64` part 报告 MIME 编码正文大小，首次落库按 `encoded × 19 / 26`（约 `× 0.730769`，整数四舍五入）估算原附件大小；非 BASE64 编码保持服务器大小。附件实际获取后由内容字节数覆盖为真实大小，历史未下载附件不迁移。`BODYSTRUCTURE` 的附件 `filename`/`name` 参数复用完整 MIME 解析器处理 RFC 2047 与 RFC 2231 扩展、分段形式。打开缺失正文时优先从本地原始 EML 重建，否则读取 `BODYSTRUCTURE` 并只获取选中的纯文本/HTML section；HTML 根据实际 CID 引用补取对应 Content-ID part，包括被服务器误标为 `application/octet-stream` 的候选，下载后仍须通过既有图片类型、文件魔数和大小预算才会内联并从附件列表移除，其余附件保持未下载。正文可用前，阅读区不展示尚未完成 CID 内联判定的附件投影；正文加载完成后才显示最终附件。加载期间正文区域只显示 spinner，失败后再显示错误与重试。启用“自动下载邮件正文”后，每个文件夹头部完成再按相同选择性路径补正文；正文回填不产生新邮件候选，Drafts 定向刷新跳过正文阶段。正文回填对服务器已消失的单封邮件跳过而非失败整条同步，本地 stub 由随后的 reconcile 修剪。`BODYSTRUCTURE` 缺失、结构无法可靠映射或当前账户已触发结构兼容降级时回退既有完整 `BODY.PEEK[]` 路径；普通网络失败不静默扩大下载范围。

HTML-only 邮件的派生纯文本与列表预览只从 `<body>` 内容提取，不包含完整 HTML 文档 `<head>` 中的 `<title>`；完整原始邮件与按 MIME section 获取正文使用同一规则。真实 `text/plain` 部分仍保持优先。

邮件正文中的 `<img>` 最大宽度由阅读器限制为 iframe 视口扣除正文留白后的宽度（含图片边框），超宽图片保持比例缩小，图片点击行为不由阅读器改写。HTML 正文左右各保留 16px 总留白，分为外层 8px 与 iframe 内 8px；正文宽度独立于滚动条计算，6px 滑块位于右侧原有留白内，不因有无溢出改变正文宽度。为保持无 scripts / same-origin 的隔离，正文通过注入受控 CSS 定制 iframe 自身滚动条，颜色复用宿主 `--muted-foreground`，有溢出时始终显示、悬停加深，不自动隐藏。
...
所有账户共享两个高层网络许可。每账户最多四条 IMAP 会话；完整同步固定占三条并为正文、附件和持久化待办保留一条交互连接。完整同步成功后，三条健康会话连同其容量许可最多保留 120 秒并由定时任务回收；期间再次同步先执行 `NOOP`，仅全部健康且账户连接配置指纹一致时复用，失败、超时、配置变化或协议解析污染都会销毁整个同步连接池并重新登录。交互请求仍按需建立且操作后关闭，不会因同步连接复用突破四条服务器连接上限。

登录后若服务器声明 `ID` 能力（RFC 2971），发送 `name=NextMail`、`version=<Cargo 包版本>`、`vendor=NextMail` 的身份标识；网易系邮箱（163/126/188）对未标识客户端拒绝邮箱操作（`Unsafe Login`）。ID 发送失败不中断连接，仅记警告。

已读、星标、移动、复制、归档和删除支持单封或同账户同文件夹批量 ID，在同一事务更新本地投影并写入 `pending_operations`。持久化待办使用独立于完整同步的账户运行循环有序重放，同步大量邮件时仍可使用第四条交互会话；邮件列表不为该后台状态显示云端同步图标，异常退出仍可恢复。缺少 UIDPLUS 时不得执行宽泛 EXPUNGE。

文件夹结构变更和全部已读必须在线成功后再事务更新本地投影。结构操作使用账户级 mailbox 路径写锁，普通同步/正文/待办/APPEND 使用读锁；锁不跨 SQLite。文件夹本地排序不改变服务器路径。

大规模同步的前端缓存边界：虚拟列表的取键回调随邮件 ID 集合变化，固定 50 封的首页轮换也会重算测量；每次列表变化清除已移出列表的尺寸记录并解除失效 DOM 观察，测量缓存随当前已加载列表规模而非历史同步量增长。正文变化只失效对应 account/message 的详情查询，不重取整个账户；正文和 EML 查询失去订阅后立即回收，已完成 Mutation 失去订阅后也立即回收，避免长时间保留正文、草稿和密码变量。后台文件夹到达队列只保留脏标记，首页溢出重读复用已有请求，卸载时清理队列并停止后续刷新。Rust 的进度快照仍逐封更新，跨 WebView 发送在同阶段/文件夹内最多每 100ms 一次，阶段切换与错误立即发送；前端缓存合并同时拒绝缓冲区内的迟到 revision。逐封落库及邮件到达事件不变。

以上边界由 2 万次首页轮换、1 万次到达突发、详情精准失效与缓存回收回归测试覆盖；Windows WebView 工作集的长期走势仍须使用真实大型邮箱实机验收，自动测试不等于进程内存实测。

### 草稿与发件

- 草稿保存 Tiptap JSON、HTML、纯文本和 revision；停止输入不会自动保存。
- Composer 的富文本编辑、HTML 预览和发件正文复用同一份安全内容样式，默认字号与行距为 `14px / 1.2`；该样式随 HTML 正文持久化和发送，显式字号、颜色、模板及签名样式继续覆盖默认值，引用原文保持独立样式范围。
...
- 客户端头为 `X-Mailer: NextMail/0.9.0`；该值由 Cargo package version 自动生成，版本变化时同步核对各 manifest。
...
- 未保存过外观偏好时默认跟随系统明暗模式；用户明确保存的系统、浅色或深色选择继续优先。
- 深色主题以 RGB 等值的零色度黑灰表面建立层级，不使用带明显蓝色偏向的底色；用户主题色只用于主操作、选中、焦点和必要状态，不得扩散为窗口背景基调。
- 全局 Lucide 图标默认使用经对比度校正后的主题主色；主操作、危险操作、成功/警告/错误状态及其他有色表面中的图标必须继承所在控件的语义前景色，保证可读性。
...
- 自绘标题栏使用无渐变的半透明语义表面、轻量模糊和永久下边界，窗口标题居中并以小型主题色标记强调，不在左上角重复；Windows 使用紧凑圆角自绘窗口控制，macOS 继续使用原生交通灯。窗口失焦时只降低标题与自绘控件强调度，仍保留标题栏边界。
- 桌面层级统一使用主题中的表面、边界、高光与阴影令牌；可调整窗格的分隔边界常驻可见，hover 与键盘 focus 只增强反馈，不承担首次发现职责。分栏拖拽命中区宽 6px、居中覆盖分隔线，与相邻滚动条命中区分离；侧栏折叠按钮隐藏时不响应鼠标，hover 或键盘焦点显示后才可点击。
...
- WebView 原生右键菜单全局禁用；只有显式接入应用 ContextMenu 的区域响应右键，禁止在空白区暴露浏览器菜单或开发工具入口。邮件详情右上角的更多菜单与邮件列表的单封右键菜单均提供 EML 另存为。
...
- `X-Priority: 1/2`、`Importance: high`、`Priority: urgent/high` 与 `X-MSMail-Priority: high` 统一视为高优先级，邮件列表主题前和阅读区标题前显示带可访问名称的 `❗`；其他值按普通优先级处理。
- 普通文件夹提交邮件搜索后，在搜索框下显示“当前文件夹/全局”范围切换；“全局”只覆盖当前账户的全部可选真实文件夹，不跨账户。全局结果保留实际来源文件夹，打开、标记和移动等操作继续作用于该来源位置。
- 邮件列表或阅读详情中的联系人右键菜单选择“编辑联系人”时，在邮件工作区直接打开共享联系人编辑器，只按联系人主键读取摘要和 revision，不切换联系人工作区，也不查询最近往来邮件；Portal 菜单操作不得冒泡到邮件行并取消当前邮件选择，“打开联系人”仍进入完整联系人详情。
- 通知总开关关闭时，显示方式、时长、账户和文件夹等全部从属选项保持可见但不可操作；总开关自身始终可重新开启。
- 文件夹列表是滚动条位置的布局例外：滚动容器向右延伸到侧栏外边距，等量 `content` 右内边距保持展开列表项宽度与折叠图标位置不变，使 6px 滑块位于列表项外侧，左右各留 5px；仍不得给 viewport 预留空间或让滑块改变列表项几何尺寸。邮件列表滑块位于行分割线右侧的 20px 空隙正中，左右各留 7px；两处滑块命中宽度均为 8px，与分栏拖拽命中区分别保留 2px / 4px 间隔。
...
- SQLite schema 1 与数据目录标记格式 1 是独立概念。
- 默认同步全部可选文件夹邮件头、`BODYSTRUCTURE`、附件元数据和每封最多 8 KiB 的文字 section 预览；同一批相同 section 的 UID 合并请求，预览不读取附件、不保存为完整正文。检测到会污染连接的非标准结构响应后，当前进程内按账户禁用 `BODYSTRUCTURE` 并有界重连，正文和附件改走完整邮件回退，列表预览使用占位符。正文预取开关不是新调度入口。
- 每账户四条 IMAP 会话预算，完整同步固定占三条并保留一条交互路径；健康同步会话可经 `NOOP` 校验跨相邻同步复用。
- 搜索支持当前账户内的当前文件夹或全部可选文件夹，不做跨账户或会话聚合；1–2 字符的无结果、稀有结果查询仍是目标范围内的顺序检查，是否需要额外短字符倒排索引应以大型真实数据库基准决定。
- 自有通知窗口不等于系统通知中心。
- Vite 主入口仍有大于 500 kB 的 chunk 警告；不要把全局拆包混入无关阶段。
- 前端没有 ESLint/Prettier，也没有普通分支或 PR CI。
- 日志在应用本地数据目录的 `logs/nextmail.log.YYYY-MM-DD`，按日滚动，最多保留 14 个日志文件；写入队列上限 1,024 条，正常退出时显式刷新。文件日志无法初始化时退回 stderr，不因日志初始化 panic。只接收 NextMail 自身 tracing target，避免 `RUST_LOG` 打开第三方协议内容日志。同一错误码/来源/原因在 60 秒内合并，下一条同类日志携带被抑制数量；去重表最多 256 项。前端上报最多 4 个并发、128 项去重状态，Rust 再验证代码与分类并限流；原始异常文本、堆栈、params 和 panic payload 均不写入日志。未分类错误保留类型和来源，不根据原始文本猜测原因。
...
- 此后不再编写开发计划。实际完成的开发统一写入 [`iterations.md`](./iterations.md)，每个自然日一个二级标题，日期倒序排列并将新日期区块插在文件顶部；每项用 `- [ ]` 和几句话概括变更及关键实现方法，只有用户明确验收后才改为 `- [x]`。
