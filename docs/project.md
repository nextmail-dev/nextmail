...
- SQLite 离线视图、全服务器邮件头与 `BODYSTRUCTURE` 优先同步、按 MIME section 获取正文、账户可选正文预取和逐封可见更新。
...
同步遍历全部可选文件夹，每条邮件头 `UID FETCH` 最多包含 20 UID，并直接逐项消费 IMAP 响应流；每收到一封就解析、原子落库并发布独立最小事件，批次中途断线时已收到的邮件不会丢失，下次按本地已有 UID 差集续传。同步开始后先把 `LIST` 返回的全部文件夹用 `ensure_mailbox`（只插入缺失行，不覆盖既有同步元数据）一次性建齐并发布变更事件，侧栏立即显示完整目录结构，再逐个文件夹同步邮件。邮件头批次进入共享动态队列，三个同步 worker 完成当前批次后立即领取下一批，避免静态连续分片中快连接等待慢连接。`BODYSTRUCTURE` 与头部拆分为同批两条命令：头部先落库，随后单独获取的 `BODYSTRUCTURE` 用于合并附件元数据；若服务器返回语法不规范且会污染 IMAP 响应流的结构（如 transfer encoding 字段为 NIL），立即废弃整个连接池并在同一轮用新连接恢复，当前进程内对该账户禁用后续 `BODYSTRUCTURE`，避免把坏连接带入下一文件夹并误判为不支持 UID。已落库邮件头按 UID 差集复用；降级账户打开邮件时直接走完整正文回退，启用自动下载正文时也直接抓取完整邮件。当前邮箱的到达事件进入最多 256 封的前端有界队列，并按动画帧逐封写入最多 50 封的首页缓存。超过上限时丢弃中间展示队列并从数据库重读一次活动首页，不丢已落库邮件。其他邮箱的到达事件按帧合并失效；联系人变化每 300ms 合并，邮件列表和详情只标记陈旧而不主动重取。同步进度仍按账户约 100ms 合并写入查询缓存并按 revision 拒绝迟到事件。进度订阅只位于侧栏子树，主窗口各窗格组件 memo 化且回调稳定；邮件列表使用虚拟化且只挂载可视行和 8 行 overscan，拖拽分隔条只重渲染布局壳层。
...
- 默认同步全部可选文件夹邮件头、`BODYSTRUCTURE` 和附件元数据；检测到会污染连接的非标准结构响应后，当前进程内按账户禁用 `BODYSTRUCTURE` 并有界重连，正文和附件改走完整邮件回退。正文预取开关不是新调度入口。
