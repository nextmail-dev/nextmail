...
- `0.7.1` 已完成并通过 Windows 实机验收：新写邮件自动插入的默认签名与正文之间补上细分隔线；模板插入不再套用编辑器盒样式、按原样呈现；Composer 默认行距从 1.65 收紧到 1.5；未下载附件在大小右侧显示云朵标记，另存为改为先选择保存位置再下载（取消不产生下载），转圈只在附件内容实际传输时出现。
