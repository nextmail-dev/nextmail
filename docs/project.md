...
- `0.8.0` 已完成并通过实机验收：侧栏新增可持久化的真实文件夹收藏，以及固定的星标邮件、未读邮件聚合入口；邮件列表支持直接切换已读状态，文件夹与邮件列表支持方向键选择；同步提示、滚动条、文件夹点击区域与对齐得到收敛；修复 HTML-only 邮件混入文档标题、重复点击当前文件夹清空阅读状态的问题，并改善新附件大小估算。
