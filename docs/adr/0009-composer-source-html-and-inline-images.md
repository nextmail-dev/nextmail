...
修订：2026-08-01，受管图片格式增加经过完整文件头验证的 BMP；`srcdoc` 构建增加历史主动内容防御；回复/转发增加稳定签名分隔和原始邮件元数据结构；原文 NodeView 只在原文/CID 输入变化时更新，签名切换使用真实 ProseMirror 节点范围替换。
...
- 原文 NodeView 缓存已经渲染的 `sourceHtml` 与 CID 预览输入；模板、签名等无关 ProseMirror transaction 不重新设置 iframe `srcdoc`。签名替换从当前文档中定位稳定签名节点并使用 ProseMirror `nodeSize` 计算真实范围，只替换签名与紧邻的签名分隔节点，不重建或覆盖原文节点。
