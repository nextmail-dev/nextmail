...
- 客户端以 4 秒上限请求 `https://api.next-mail.app/api/v1/geo`。只有成功解析并明确返回 `country_code: CN` 时选择代理后的 `latest-cn.json`；非 CN、超时、非成功状态或无效响应都回退 GitHub `latest.json`。地理结果不持久化、不写日志、不进入账户数据。
