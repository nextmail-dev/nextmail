...
状态：实施中。第一批“语料与安全决策”已于 2026-07-21 验收通过；第二批“阅读器保真与深色模式”待实施。依赖第九阶段已验收的签名节点与默认规则。
...
- Rust 清洗回归对完整语料固定严格 CSP、脚本/事件/表单/嵌入文档、危险 scheme、CSS 网络资源和固定遮罩边界；前端测试复用同一语料并继续精确验证 `sandbox=""`、无 `allow` 与 `no-referrer`。
- 新增提议 ADR 0008：保留不透明 sandbox iframe；CSS 采用解析后重建的展示子集；深色样式只做可被作者明确配色覆盖的兜底；外链使用账户/消息归属的不透明 ID、自定义协议窄桥接、离站确认和 Rust 系统打开。
- 没有改变生产 HTML 清洗、主题、远程图片、外链、Capability、IPC 或回复/转发行为；第二至四批范围保持不变。
- `cssparser 0.37` 已由现有依赖间接进入锁文件，但其 MPL-2.0 许可证不属于项目默认宽松许可证集合。第二批若直接采用，必须随本批由用户明确确认；未确认前不新增直接依赖。
