...
状态：已验收。第三阶段基础写信/草稿/SMTP 能力此后继续由第四、九、十、二十和二十一阶段扩展；当前事实以 `docs/project.md` 和代码为准。
...
本轮没有实现 IMAP Sent/Drafts 上传、可配置签名库、模板、多账户或完整发件箱页面。
...
本阶段继续限制为首次启动时保存的单账户和密码认证。多账户、模板/签名管理、IMAP Sent/Drafts 写入和离线 IMAP 操作属于后续阶段。
...
6. 更新长期项目手册和本阶段变更摘要，完成自动验证后交付手动验收。
...
- 0007：实现独立 Composer、三格式草稿、附件、MIME 和持久化 SMTP 发件。
- 0008：修复写信窗口生命周期、主界面层级和邮件阅读器问题。
- 0009：精修连续邮件列表、侧栏写信入口和编辑器焦点。
- 0010：增加草稿删除确认并修正邮件列表顶部对齐。
