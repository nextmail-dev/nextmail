...
本阶段以一次外部代码审查结果为输入，在不增加产品功能、不改变稳定 Command DTO、错误码和安全边界的前提下，分批修复正确性问题、同步与存储热路径、分层泄漏、前端状态重复和工具链缺口。审查建议已在 P0/P1 Rust 完成后归档删除，剩余有效工作已收敛到 `plans/next-implementation.md`。
