...
状态：进行中。P0、P1 Rust 分层与 P1 前端结构均已验收；P2/P3 未排期。

## 目标

本阶段以一次外部代码审查结果为输入，在不增加产品功能、不改变稳定 Command DTO、错误码和安全边界的前提下，分批修复正确性问题、同步与存储热路径、分层泄漏、前端状态重复和工具链缺口。审查建议已在 P0/P1 Rust 完成后归档删除，剩余候选工作直接在本文维护状态。

本阶段仍遵循单一 `src-tauri` Cargo package，不恢复根 Cargo Workspace。每一批完成自动验证后单独交付手动验收；未经确认不进入下一批，也不提交。

## 实施批次

1. **P0 正确性与热路径（已验收）**：F1、R1–R4。
2. **P1 Rust 分层（已验收）**：R5–R13，先上移回复/转发用例，再拆 Repository、注入 ports，最后拆 IMAP 编排。
3. **P1 前端结构（已验收）**：F2–F4，合并外观偏好数据源，并拆分主工作区状态、事件与分栏 hook。
4. **P2/P3 性能与清理（未排期）**：F5–F12、R14–R19、T1–T2；范围需要后续单独确认，工具链是否加入远程 CI 仍受“不配置远程仓库”约束。
...
- 外观偏好删除 Zustand 副本，统一由各 WebView 自身的 TanStack Query cache 持有；写入通过 mutation 乐观更新，失败回滚，成功以 Rust 持久化返回值覆盖。
- 外观事件桥只同步当前窗口 Query cache 与 DOM 主题；主窗口、设置窗口和写信窗口继续通过 Rust 事件同步，不共享 React 内存。
- 写信窗口关闭监听只按稳定账户/草稿身份订阅一次，最新保存函数、脏状态和发件状态由 ref 提供；保存失败仍阻止关闭，空白草稿规则不变。

### 主工作区 hooks

- `useMailboxSelection` 管理账户、默认文件夹、邮件与搜索选择，并在账户/文件夹变化时清理下游选择。
- `useMailRuntimeEvents` 管理 mailbox、message、sync、send-job 和 pending-operation 监听，使用集中 Query key 工厂失效账户或文件夹范围；账户切换不重建监听。
- `usePaneLayout` 管理文件夹栏折叠、两条分栏、窗口 resize、宽度钳制与标题栏 CSS 令牌，使用函数式状态更新避免陈旧宽度闭包。
- `MainShell` 保留数据查询、业务操作和三栏组合，不再包含事件 wiring 或底层 resize 数学。

### 第三批自动验收

- `pnpm test`：19 个测试文件、39 项测试全部通过。
- `pnpm build`：TypeScript 与 Vite 生产构建通过；既有主入口 chunk 大小提示保留给后续独立批次。
- `cargo fmt --all -- --check` 通过。
- `cargo test --offline --locked`：58 项测试全部通过。
- `cargo clippy --offline --locked --all-targets -- -D warnings` 通过。
- `git diff --check` 通过；未执行 Tauri bundle，正常增量缓存继续保留。

### 第三批手动验收

1. 设置窗口连续切换中文/英文、浅色/深色/系统和多个主题色，确认主窗口与写信窗口立即同步，重启后保持。
2. 写信时连续输入并关闭，重新打开确认草稿完整；完全空白草稿不残留。
3. 拖动两条分栏、折叠文件夹栏并缩放窗口，确认宽度钳制和视觉反馈不变。
4. 验证后台同步、附件、Flags、移动/复制/删除、草稿与发件事件仍及时刷新。
