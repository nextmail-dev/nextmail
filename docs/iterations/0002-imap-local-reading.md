...
- Rust Ammonia 清洗、严格 CSP 与无脚本 sandbox iframe；远程图片默认阻止，可由用户本次放行或通过设备级阅读偏好自动加载，外链与 CID 在未有受控协议前仍不会导航或加载。

与原计划的待确认差异：

- “始终允许发件人”的远程图片代理、受控外链、CID 资源协议尚未启用；当前“本次加载”和设备级自动加载直接放宽安全 iframe 的 HTTP(S) 图片 CSP，并继续使用 sandbox 与 `no-referrer`。
