...
- 全局或账户范围的富文本邮件模板与签名库 CRUD；第九阶段第一批已通过 Windows 手动验收。
...
- 模板变量、四种写信场景默认规则、Composer 模板/签名选择与稳定签名节点。
...
`application` 组织首次启动、账户生命周期和纯业务用例。账户/Bootstrap/外观/阅读配置通过 trait 注入；回复/转发的收件人、主题、引用块与编辑器内容生成位于 `application/message_composer.rs`。模板/签名名称、主题长度和三格式内容校验位于 `application/composition_definitions.rs`，不进入 SQLx Repository。
...
- 模板与签名库：按全局或账户范围列出、新建、更新和删除富文本定义；账户 ID 在 Rust 内转换为匿名数据槽。
...
- `mail_templates`、`mail_signatures`
- `pending_operations`
...
设置窗口“写信”分类提供范围切换、富文本新建/编辑和二次确认删除。当前第一批只管理定义库，不自动插入 Composer；变量、默认规则、稳定签名节点和四种写信场景接入属于第九阶段第二批。
