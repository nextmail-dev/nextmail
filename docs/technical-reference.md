...
- 会话聚合、跨账户搜索、统一收件箱。
- 托盘、可见桌面通知窗口、系统通知中心集成、自动更新与正式发布流水线；通知偏好与新邮件候选判定已经实现。
...
普通邮件事件仅包含账户、文件夹、消息/任务/操作 ID、状态、阶段进度或修订号；界面收到事件后重新读取本地视图。`message-body-progress` 不携带正文，只描述用户手动请求的准备、下载、处理、更新与完成阶段。`new-mail-candidate` 是后续受限通知窗口的最小输入，只包含公开账户/文件夹/消息 ID、首个发件人姓名和地址及主题，不包含正文、预览、附件、内部路径、凭据或服务器错误。
...
邮件规范记录与远端位置分离，同一邮件可位于多个文件夹。原始 EML 是重新解析来源；正文、附件元数据和 FTS 索引可重建。迁移 0016 增加非收件箱正文偏好及动作草稿编辑标记；迁移 0017 新增默认签名偏好，并把既有场景签名引用按新建、回复、回复全部、转发顺序收敛成单一默认值；迁移 0018 为匿名账户槽增加通知同步基线，并把已经成功同步过的既有账户标记为就绪。迁移只能新增到 `src-tauri/migrations`，已发布迁移不得修改。
