...
`application` 组织首次启动、账户生命周期和纯业务用例。账户/Bootstrap/外观/阅读配置通过 trait 注入；回复/转发的收件人、主题、引用块与编辑器内容生成位于 `application/message_composer.rs`。模板/签名名称、主题长度、变量白名单、按上下文转义和初始三格式组合位于 `application/composition_definitions.rs`，不进入 SQLx Repository。
...
设置窗口“写信”分类提供范围切换、富文本定义管理和四种默认场景规则。Composer 可显式选择当前账户可见的全局/账户定义；模板与签名分别写入 `nextmailTemplate` / `nextmailSignature` 块节点，并把定义 ID 同步到 HTML 的 `data-nextmail-*-id` 属性。切换只替换同类节点，用户删除签名后保存和重开不会自动恢复。初始新建或回复/转发创建时才解析默认规则；远端既有草稿不重新套用默认值。三格式仍由 Tiptap 更新并进入既有远端 Drafts 与 MIME 流程。
...
- HTML 在 Rust 端删除脚本、事件、表单、iframe、危险 URL 和外部样式表。`<style>` 与行内声明都通过 CSS parser 重建，只保留展示属性、普通/属性选择器及受控响应式/深色媒体查询；`class`、`id` 和传统表格邮件的安全宽高/间距/对齐/背景色/字体属性继续保留。
...
- 阅读器不再注入统一字体/行高、16px 内边距、任意断词或图片/表格最大宽度，避免覆盖作者固定宽度和居中布局。迁移 0011 作为可能已应用的原型保持不可变，0012 删除临时链接表并失效旧缓存。
- 第十阶段共享语料持续覆盖样式保真和主动内容边界。ADR 0008 已接受并根据第一次第三批实机失败修订；直接外链与传统布局修正已于 2026-07-21 通过 Windows WebView2 手动验收，macOS 未执行。
...
- CID/内联附件协议和远程图片代理仍未完成，是否纳入需单独确认。CSS 背景图、Web Font 等资源当前统一丢弃，不随“显示远程图片”加载。
