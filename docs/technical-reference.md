...
本文描述仓库当前代码已经实现并通过验收的技术状态。历史实施过程见 `iterations/` 和 `changes/`，长期架构理由见 `adr/`，后续工作见 `plans/next-implementation.md`。
...
- TanStack Query 5 负责服务端/本地视图缓存，Zustand 当前仍用于外观状态，后续计划收敛为单一数据源。
...
│  ├─ features/                onboarding/accounts/mail/composer/preferences
...
- 外观偏好仍同时存在 Zustand 与 TanStack Query 数据源。
- `MainShell` 的选择、事件和分栏布局状态仍需抽为 hooks。
- 写信窗口关闭监听会随编辑状态重建，需稳定为单次订阅。
...
上述内容的实施顺序和验收边界见 `plans/next-implementation.md`。
