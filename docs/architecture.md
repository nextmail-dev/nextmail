...
- `src-tauri/src/storage`：共享 SQLite/内容存储之上的读取、当前文件夹全文搜索、同步写入、模板/签名、草稿、发件任务、待办操作和文件夹角色子仓库；同步写入与发件任务分别有窄实现文件，嵌入式迁移位于 `src-tauri/migrations`。
- `src-tauri/src/protocols`：IMAP 同步与写操作、MIME 解析/生成和 HTML 清洗；IMAP 内部按 Provider、连接、账户会话预算、具体会话操作、解析、文件夹编码和超时流拆分。新增协议 Adapter 只有在未来单独排期后进入此边界。
...
主窗口账户菜单内的管理弹窗提供账户列表、添加、连接编辑、重新认证、手动/1/5/10 分钟自动同步间隔、文件夹映射和安全移除；账户列表与详情面板仍由 TanStack Query 驱动，账户或运行状态变更只失效对应 key。独立设置窗口不再保留重复的“账户”分类，“写信”分类继续提供全局/账户模板与签名库的富文本管理；“通知”分类提供全局开关、展示模式/数量/时间、逐账户开关及三点菜单内的逐文件夹开关，文件夹长列表固定保留滚动槽并始终显示滚动条。偏好写入会关闭当前瞬时通知，后续候选使用新设置；高级类别仍为稳定占位。首次启动与账户管理弹窗复用同一个密码账户表单和发现/手动配置流程，不维护两套连接验证逻辑。移除最后一个账户后保留已经完成的数据目录初始化，主窗口展示打开同一账户管理弹窗的正式添加入口，不重新进入数据目录向导。
