...
文件夹栏和邮件列表可在最小/最大宽度内拖动，拖拽区默认无痕并在 hover/键盘聚焦时显示主题自适应提示条；文件夹栏可折叠为保留可访问名称的图标模式。当前搜索只过滤已加载邮件，不等同于后续 FTS 全文搜索。设置不再使用主窗口模态框，而是独立分类窗口；通用、外观、账户和关于已接入现有能力，阅读、写信、通知和高级类别先提供稳定占位。
